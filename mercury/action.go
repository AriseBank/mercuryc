...
func (c *actionCmd) usage() string {
	extra := ""
	if c.name == "pause" {
		extra = "\n" + i18n.G("The opposite of \"mercury pause\" is \"mercury start\".")
	}

	return fmt.Sprintf(i18n.G(
		`Usage: mercury %s [<remote>:]<container> [[<remote>:]<container>...]
...
func (c *actionCmd) doAction(conf *config.Config, nameArg string) error {
	...
	err = op.Wait()
	if err != nil {
		return fmt.Errorf("%s\n"+i18n.G("Try `mercury info --show-log %s` for more info"), err, nameArg)
	}
