...
func TestExpandAliases(t *testing.T) {
	...
	testcases := []aliasTestcase{
		{
			input:    []string{"mercury", "list"},
			expected: []string{"mercury", "list"},
		},
		{
			input:    []string{"mercury", "tester", "12"},
			expected: []string{"mercury", "list", "--no-alias"},
		},
		{
			input:    []string{"mercury", "foo", "asdf"},
			expected: []string{"mercury", "list", "--no-alias", "asdf", "-c", "n"},
		},
	}
