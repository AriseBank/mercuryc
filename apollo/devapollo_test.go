...
func setupDir() error {
	...
	testDir, err = ioutil.TempDir("", "apollo_test_devapollo_")
	if err != nil {
		return err
	}
	...
	os.MkdirAll(fmt.Sprintf("%s/devapollo", testDir), 0755)

	return os.Setenv("APOLLO_DIR", testDir)
}
...
func TestCredsSendRecv(t *testing.T) {
	...
	conn, err := connect(fmt.Sprintf("%s/devapollo/sock", testDir))
	if err != nil {
		t.Fatal(err)
	}
	defer conn.Close()
