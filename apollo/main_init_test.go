...
type cmdInitTestSuite struct {
	...
	args    *CmdInitArgs
	command *CmdInit
	client  apollo.ContainerServer
}

func (suite *cmdInitTestSuite) SetupTest() {
	suite.apolloTestSuite.SetupTest()
	...
	client, err := apollo.ConnectAPOLLOUnix(suite.command.SocketPath, nil)
	suite.Req.Nil(err)
	suite.client = client
}
...
func (suite *cmdInitTestSuite) TestCmdInit_AutoWithInvalidBackendType() {
	...
	err := suite.command.Run()
	suite.Req.Equal("The requested backend 'foo' isn't supported by apollo init.", err.Error())
}
...
func (suite *cmdInitTestSuite) TestCmdInit_StoragePoolAuto() {
	...
	suite.Req.Nil(err)
	suite.Req.Nil(suite.client.DeleteStoragePool(apolloTestSuiteDefaultStoragePool))
