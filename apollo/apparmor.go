...
#  mount options=(rw,make-runbindable) -> **,
  ...
  mount options=(rw,bind) /dev/.mercury?*{,/**},
  ...
  mount options=(rw,move) /dev/.mercury?*{,/**},
  ...
  # generated by: mercury-generate-aa-rules.py container-rules.base
...
const AA_PROFILE_NESTING = `
  ...
  deny /dev/.apollo/proc/** rw,
  deny /dev/.apollo/sys/** rw,

  mount /var/lib/apollo/shmounts/ -> /var/lib/apollo/shmounts/,
  mount none -> /var/lib/apollo/shmounts/,
  mount fstype=proc -> /usr/lib/*/mercury/**,
  mount fstype=sysfs -> /usr/lib/*/mercury/**,
  ...
  mount,
  mount options=bind /var/lib/apollo/shmounts/** -> /var/lib/apollo/**,
`
...
func AANamespace(c container) string {
	 ...
	 * leading / so it doesn't look like "-var-lib-apollo"
	 */
	apollodir := strings.Replace(strings.Trim(shared.VarPath(""), "/"), "/", "-", -1)
	apollodir = mkApparmorName(apollodir)
	return fmt.Sprintf("apollo-%s_<%s>", c.Name(), apollodir)
}
...
func AAProfileShort(c container) string {
	return fmt.Sprintf("apollo-%s", c.Name())
}

// getProfileContent generates the apparmor profile template from the given
// container. This includes the stock mercury includes as well as stuff from
// raw.apparmor.
