...
#define APOLLO_MAXPATH 4096
#define APOLLO_NUMSTRLEN64 21
#define APOLLO_MAX_LOOP_PATHLEN (2 * sizeof("loop/")) + APOLLO_NUMSTRLEN64 + sizeof("backing_file") + 1
...
static int find_associated_loop_device(const char *loop_file,
				       char *loop_dev_name)
{
	char looppath[APOLLO_MAX_LOOP_PATHLEN];
	char buf[APOLLO_MAXPATH];
	struct dirent *dp;
	DIR *dir;
	int dfd = -1, fd = -1;
