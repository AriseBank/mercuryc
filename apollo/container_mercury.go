...
// Loader functions
func containerMERCURYCreate(d *Daemon, args containerArgs) (container, error) {
...
func containerMERCURYLoad(d *Daemon, args containerArgs) (container, error) {
	// Create the container struct
	c := &containerMERCURY{
		daemon:       d,
		id:           args.Id,
...
func (c *containerMERCURY) initMERCURY() error {
	...
	err = mercurySetConfigItem(cc, "mercury.pts", "1024")
	if err != nil {
		return err
	}
	...
	err = mercurySetConfigItem(cc, "mercury.loglevel", logLevel)
	if err != nil {
		return err
	}
	...
	// Setup the console
	err = mercurySetConfigItem(cc, "mercury.tty", "0")
	if err != nil {
		return err
	}

	// Setup the hostname
	err = mercurySetConfigItem(cc, "mercury.utsname", c.Name())
	if err != nil {
		return err
	}
	...
	if aaAvailable {
		if aaConfined || !aaAdmin {
			...
			err = mercurySetConfigItem(cc, "mercury.aa_profile", curProfile)
			if err != nil {
				return err
			}
		} else {
			...
			err := mercurySetConfigItem(cc, "mercury.aa_profile", profile)
			if err != nil {
				return err
			}
		}
	...
	if ContainerNeedsSeccomp(c) {
		err = mercurySetConfigItem(cc, "mercury.seccomp", SeccompProfilePath(c))
		if err != nil {
			return err
		}
	}
	...
	for _, k := range c.expandedDevices.DeviceNames() {
		...
		if shared.StringInSlice(m["type"], []string{"unix-char", "unix-block"}) {
			...
			if tgtPath == "" {
				...
				// Set the rootfs path
				err = mercurySetConfigItem(cc, "mercury.rootfs", c.RootfsPath())
				if err != nil {
					return err
				}
...
func (c *containerMERCURY) Restore(sourceContainer container) error {
	var ctxMap log.Ctx
...
func (c *containerMERCURY) Update(args containerArgs, userRequested bool) error {
...
func (c *containerMERCURY) FilePush(srcpath string, dstpath string, uid int64, gid int64, mode int, write string) error {
	var rootUid int64
	var rootGid int64
	var errStr string
