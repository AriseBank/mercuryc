...
func TestVersion(t *testing.T) {
	t.Logf("MERCURY version: %s", Version())
}
...
func TestConfigItem(t *testing.T) {
	...
	if c.ConfigItem("mercury.utsname")[0] != ContainerName {
...
func TestSetConfigItem(t *testing.T) {
	...
	if err := c.SetConfigItem("mercury.utsname", ContainerName); err != nil {
		t.Errorf(err.Error())
	}
...
func TestRunningConfigItem(t *testing.T) {
	...
	if c.RunningConfigItem("mercury.network.0.type") == nil {
...
func TestClearConfigItem(t *testing.T) {
	...
	if err := c.ClearConfigItem("mercury.cap.drop"); err != nil {
		t.Errorf(err.Error())
	}
	if c.ConfigItem("mercury.cap.drop")[0] != "" {
...
func TestSupportedConfigItems(t *testing.T) {
	if VersionAtLeast(2, 1, 0) {
		if !IsSupportedConfigItem("mercury.arch") {
			t.Errorf("IsSupportedConfigItem failed to detect \"mercury.arch\" as supported config item...")
		}

		if IsSupportedConfigItem("mercury.nonsense") {
			t.Errorf("IsSupportedConfigItem failed to detect \"mercury.nonsense\" as unsupported config item...")
		}
	}
}
