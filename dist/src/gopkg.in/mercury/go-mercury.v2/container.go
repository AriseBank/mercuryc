...
func (c *Container) InterfaceStats() (map[string]map[string]ByteSize, error) {
	...
	for i := 0; i < len(c.ConfigItem("mercury.network")); i++ {
		interfaceType := c.RunningConfigItem(fmt.Sprintf("mercury.network.%d.type", i))
		if interfaceType == nil {
			continue
		}

		if interfaceType[0] == "veth" {
			interfaceName = c.RunningConfigItem(fmt.Sprintf("mercury.network.%d.veth.pair", i))[0]
		} else {
			interfaceName = c.RunningConfigItem(fmt.Sprintf("mercury.network.%d.link", i))[0]
		}
...
func (c *Container) SetLogFile(filename string) error {
	if err := c.SetConfigItem("mercury.logfile", filename); err != nil {
		return err
	}
	return nil
}
...
func (c *Container) SetLogLevel(level LogLevel) error {
	if err := c.SetConfigItem("mercury.loglevel", level.String()); err != nil {
		return err
	}
	return nil
}
