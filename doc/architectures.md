# Introduction
APOLLO just like MERCURY can run on just about any architecture that's
supported by the Linux kernel and by Go.

Some objects in APOLLO are tied to an architecture, like the container,
...
Please note that what APOLLO cares about is the kernel architecture, not
the particular userspace flavor as determined by the toolchain.

That means that APOLLO considers armv7 hard-float to be the same as armv7
