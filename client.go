...
// DEPRECATED: This package is now deprecated in favor of github.com/AriseBank/apollo-controller/client
//
// This package will be entirely removed from the code tree with APOLLO 2.16
...
// Client can talk to a APOLLO daemon.
type Client struct {
	BaseURL     string
	BaseWSURL   string
	Config      Config
...
var (
	// APOLLOErrors are special errors; the client library hoists error codes
	...
	// APOLLO passes an error string along which is more informative than
	// whatever static error message we would put here.
	APOLLOErrors = map[int]error{
		http.StatusNotFound: fmt.Errorf("not found"),
	}
)

// ParseResponse parses a apollo style response out of an http.Response. Note that
...
// NewClient returns a new APOLLO client.
func NewClient(config *Config, remote string) (*Client, error) {
	...
	if strings.HasPrefix(r.Addr, "unix:") {
		// replace "unix://" with the official "unix:/var/lib/apollo/unix.socket"
...
// ConnectInfo contains the information we need to connect to a specific APOLLO server
type ConnectInfo struct {
	// Name is a simple identifier for the remote server. In 'mercury' it is
...
// NewClientFromInfo returns a new APOLLO client.
...
// GetLocalAPOLLOErr determines whether or not an error is likely due to a
// local APOLLO configuration issue, and if so, returns the underlying error.
// GetLocalAPOLLOErr can be used to provide customized error messages to help
// the user identify basic system issues, e.g. APOLLO daemon not running.
//
// Returns syscall.ENOENT, syscall.ECONNREFUSED or syscall.EACCES when a
// local APOLLO configuration issue is detected, nil otherwise.
func GetLocalAPOLLOErr(err error) error {
	t, ok := err.(*url.Error)
	if !ok {
		return nil
	}
...
func (c *Client) PostImage(imageFile string, rootfsFile string, properties []string, public bool, aliases []string, progressHandler func(int64, int64)) (string, error) {
	...
	if rootfsFile != "" {
		...
		body, err := ioutil.TempFile("", "mercury_image_")
		if err != nil {
			return "", err
		}
		...
		req.Header.Set("X-APOLLO-filename", filepath.Base(imageFile))
	...
	if public {
		req.Header.Set("X-APOLLO-public", "1")
	} else {
		req.Header.Set("X-APOLLO-public", "0")
	}
...
func (c *Client) IsAlias(alias string) (bool, error) {
	...
	if err != nil {
		if err == APOLLOErrors[http.StatusNotFound] {
			return false, nil
		}
		return false, err
	}
...
func (c *Client) Init(name string, imgremote string, image string, profiles *[]string, config map[string]string, devices map[string]map[string]string, ephem bool) (*api.Response, error) {
	...
	if err != nil {
		if APOLLOErrors[http.StatusNotFound] == err {
...
// Exec runs a command inside the APOLLO container. For "interactive" use such as
// `mercury exec ...`, one should pass a controlHandler that talks over the control
...
func (c *Client) PushFile(container string, p string, gid int, uid int, mode string, buf io.ReadSeeker) error {
	...
	req.Header.Set("User-Agent", version.UserAgent)
	req.Header.Set("X-APOLLO-type", "file")

	if mode != "" {
		req.Header.Set("X-APOLLO-mode", mode)
	}
	if uid != -1 {
		req.Header.Set("X-APOLLO-uid", strconv.FormatUint(uint64(uid), 10))
	}
	if gid != -1 {
		req.Header.Set("X-APOLLO-gid", strconv.FormatUint(uint64(gid), 10))
	}
...
func (c *Client) Mkdir(container string, p string, mode os.FileMode, uid int, gid int) error {
	...
	req.Header.Set("User-Agent", version.UserAgent)
	req.Header.Set("X-APOLLO-type", "directory")
	req.Header.Set("X-APOLLO-mode", fmt.Sprintf("%04o", mode.Perm()))
	if uid != -1 {
		req.Header.Set("X-APOLLO-uid", strconv.FormatUint(uint64(uid), 10))
	}
	if gid != -1 {
...
func (c *Client) PullFile(container string, p string) (int64, int64, int, string, io.ReadCloser, []string, error) {
	...
	uid, gid, mode, type_, _ := shared.ParseAPOLLOFileHeaders(r.Header)
	if type_ == "directory" {
...
// WebsocketDial attempts to dial a websocket to a APOLLO instance, parsing
// APOLLO-style errors and returning them as go errors.
