test_database_update(){
  APOLLO_MIGRATE_DIR=$(mktemp -d -p "${TEST_DIR}" XXX)
  MIGRATE_DB=${APOLLO_MIGRATE_DIR}/apollo.db

  # Create the version 1 schema as the database
  sqlite3 "${MIGRATE_DB}" > /dev/null < deps/schema1.sql

  # Start an APOLLO demon in the tmp directory. This should start the updates.
  spawn_apollo "${APOLLO_MIGRATE_DIR}" true
  ...
  kill_apollo "$APOLLO_MIGRATE_DIR"
}
