...
test_nonroot_fuidshift() {
  ...
  APOLLO_FUIDMAP_DIR=$(mktemp -d -p "${TEST_DIR}" XXX)
  ...
  touch "${APOLLO_FUIDMAP_DIR}/x1"
  fuidshift "${APOLLO_FUIDMAP_DIR}/x1" -t "u:${u}:100000:1" "g:${g}:100000:1" | tee /dev/stderr | grep "to 100000 100000" > /dev/null || fail=1
  if [ "${fail}" -eq 1 ]; then
    echo "==> Failed to shift own uid to container root"
    false
  fi
  fuidshift "${APOLLO_FUIDMAP_DIR}/x1" -t "u:${u1}:10000:1" "g:${g1}:100000:1" | tee /dev/stderr | grep "to -1 -1" > /dev/null || fail=1
  if [ "${fail}" -eq 1 ]; then
  ...
  # unshift it
  chown 100000:100000 "${APOLLO_FUIDMAP_DIR}/x1"
  fuidshift "${APOLLO_FUIDMAP_DIR}/x1" -r -t "u:${u}:100000:1" "g:${g}:100000:1" | tee /dev/stderr | grep "to 0 0" > /dev/null || fail=1
  if [ "${fail}" -eq 1 ]; then
