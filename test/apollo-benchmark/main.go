...
func spawnContainers(c apollo.ContainerServer, count int, image string, privileged bool) error {
	batch := *argParallel
	if batch < 1 {
	...
	startContainer := func(name string) {
		...
		config["user.apollo-benchmark"] = "true"
...
func deleteContainers(c apollo.ContainerServer) error {
	batch := *argParallel
	if batch < 1 {
		// Detect the number of parallel actions
		cpus, err := ioutil.ReadDir("/sys/bus/cpu/devices")
	...
	for _, container := range allContainers {
		if container.Config["user.apollo-benchmark"] != "true" {
			continue
		}
